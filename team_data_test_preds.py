...
class FilesEncodedDataset(BaseDataset):
    ...
    def get_x(self, i): return open_image(path/self.fnames[i])
    def get_y(self, i): 
        
        mask = np.load(path/self.fnames2[i]).astype(float)
        return mask
...
dataset = ImageData.get_ds(FilesEncodedDataset, 
                           ...
                           test=(TEST_SUB_X, TRN_Y[:len(TEST_SUB_X)]) ,path=path)
...
def tta_preds(x, model, mode="max"):
    ...
    for i, (out1_i, out2_i, out3_i, out4_i, out5_i, out6_i, out7_i) in enumerate(zip(out1_probas, out2_probas,
        ...
        if mode == "max":
            ...
            outs.append(out1_i.astype(np.float32))
    return outs


os.makedirs(path/f"team_data/stacking/test_preds_{FOLD}", exist_ok=True)

model = learn.model.eval()
print(f"Predicting test: {len(learn.data.test_ds)} with FOLD: {FOLD}")
fnames = [fname.split("/")[-1] for fname in learn.data.test_ds.fnames]
idx = 0

for i, (x,y) in enumerate(learn.data.test_dl):
    if i % 100 == 0: print(i)
    preds = tta_preds(x, model, mode='mean')
    for pred in preds:
        pred = np.array(pred)*255
        cv2.imwrite(str(path/f"team_data/stacking/test_preds_{FOLD}/{fnames[idx]}.png"), pred)
        idx += 1
